...
- **Status Standarisasi Tombol Aksi Tabel Modern (`<x-menu-aksi-tabel>`):** ✅ **100% SELESAI & TERAPLIKASI DI SELURUH TABEL SISTEM** (Komponen Three-Dots Menu Popover `•••` berbasis Alpine.js dengan penulisan label ringkas & padat: `Salin Kode`, `Detail`, `Edit`, `Hapus`, `Cetak`, `Cetak Voucher`, `Cetak Lembar SO`, `Bayar`, `Ubah Status`, `Mutasi Stok`, notifikasi toast salin cepat, proteksi Hak Akses RBAC & Read-Only Guard `apakahReadOnly(modul)`. Evaluasi unescaped raw `{!! $aksiEdit !!}` dan arsitektur event-driven `$dispatch('buka-edit-...', 'KODE')` menjamin kebal terhadap *double-escaping* JSON, serta deteksi batas kontainer `ruangBawahKontainer < 180` yang otomatis memicu `bukaKeAtas` agar menu popover tidak terpotong oleh batas bawah kontainer `overflow-x-auto` ataupun pagination bar).
