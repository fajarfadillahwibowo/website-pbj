...
- **Status Seleksi Multi-Baris & Floating Bar Aksi Massal Cerdas Berbasis Hak Akses (`<x-bar-aksi-massal>` & Checkboxes):** ✅ **100% SELESAI, TERINTEGRASI & TERUJI AMAN** (Fitur memilih lebih dari 1 baris menggunakan checkbox *Select All* pada header `<th>`, checkbox individual per baris `<td>`, pewarnaan baris terpilih `:class`, serta bar aksi melayang di bagian bawah. Khusus peran Read-Only / Lihat Saja, seluruh kolom checkbox dan bar aksi disembunyikan secara otomatis. Untuk pengguna berwenang penuh, floating bar dilengkapi tombol **"Hapus Terpilih"** dengan modal konfirmasi dan 9 controller backend berproteksi relasi foreign key & saldo aktif, serta tombol **"Salin Terpilih"**).
