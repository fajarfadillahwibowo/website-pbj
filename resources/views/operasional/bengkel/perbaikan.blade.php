...
<div x-data="kelolaPerbaikanKendaraan()" x-init="initPerbaikan()" class="space-y-6">
        ...
        <div class="p-4 sm:px-5 sm:py-4 border-b border-[#E2E8F0] dark:border-[#252837] flex flex-col md:flex-row md:items-center justify-between gap-3">
            <form method="GET" action="{{ route('operasional.bengkel.perbaikan') }}" class="flex flex-col sm:flex-row items-stretch sm:items-center gap-2.5 flex-1 max-w-2xl">
                ...
                <select name="status" onchange="this.form.submit()"
                        class="px-3 py-2 text-xs rounded-xl bg-[#F8FAFC] dark:bg-[#1C1E2A] border border-[#E2E8F0] dark:border-[#252837] text-slate-800 dark:text-slate-200 focus:outline-none focus:ring-2 focus:ring-red-500/30">
                    <option value="semua" {{ ($statusFilter ?? 'semua') === 'semua' ? 'selected' : '' }}>Semua Status</option>
                    <option value="Dalam Proses" {{ ($statusFilter ?? '') === 'Dalam Proses' ? 'selected' : '' }}>Dalam Proses</option>
                    <option value="Selesai" {{ ($statusFilter ?? '') === 'Selesai' ? 'selected' : '' }}>Selesai</option>
                    <option value="Menunggu Sparepart" {{ ($statusFilter ?? '') === 'Menunggu Sparepart' ? 'selected' : '' }}>Menunggu Sparepart</option>
    ...
    <!-- ========================================================================= -->
    <!-- 5. MODAL FORM: BUAT SPK PERBAIKAN -->
    <!-- ========================================================================= -->
    <div x-show="modalTambahTerbuka" x-cloak
         class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-xs overflow-y-auto">
        <div @click.away="modalTambahTerbuka = false"
             class="bg-white dark:bg-[#14161F] border border-[#E2E8F0] dark:border-[#252837] rounded-2xl w-full max-w-xl overflow-hidden shadow-2xl my-8">
            
            <div class="flex items-center justify-between px-6 py-4 border-b border-[#E2E8F0] dark:border-[#252837]">
                <div class="flex items-center gap-2.5">
                    <div class="w-8 h-8 rounded-xl bg-red-50 dark:bg-red-500/10 text-red-600 dark:text-red-400 flex items-center justify-center font-bold">
                        <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5"><path stroke-linecap="round" stroke-linejoin="round" d="M12 4v16m8-8H4"/></svg>
                    </div>
                    <div>
                        <h2 class="text-base font-bold text-slate-900 dark:text-slate-100">Terbitkan Surat Perintah Kerja (SPK)</h2>
                        <p class="text-[11px] text-slate-400">Pencatatan perbaikan armada truk dan rincian biaya bengkel.</p>
                    </div>
                </div>
            ...
            <form action="{{ route('operasional.bengkel.perbaikan.simpan') }}" method="POST" class="p-6 space-y-4">
                @csrf
                ...
                <div class="p-3.5 rounded-xl bg-slate-50 dark:bg-[#1C1E2A] border border-[#E2E8F0] dark:border-[#252837]">
                    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-2 mb-2">
                        <div>
                            <label class="text-xs font-bold text-slate-800 dark:text-slate-200">
                                Nomor SPK Perbaikan <span class="text-rose-500">*</span>
                            </label>
                            <div class="text-[10px] text-slate-400 font-mono mt-0.5" x-text="keteranganKodeSPK"></div>
                        </div>
                        
                        <div class="flex items-center gap-1.5 shrink-0">
                            <button type="button" @click="buatNomorSPK('gap')"
                                    class="px-2.5 py-1 text-[11px] font-semibold text-red-700 dark:text-red-300 bg-red-100 dark:bg-red-900/30 hover:bg-red-200 rounded-lg transition-colors flex items-center gap-1 shadow-xs">
                                <svg class="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15"/></svg>
                                <span>Daur Ulang (SPK-001)</span>
                            </button>
                            <button type="button" @click="buatNomorSPK('acak')"
                    ...
                    <input type="text" name="nomor_spk_perbaikan" x-model="formTambah.nomor_spk_perbaikan" required placeholder="Contoh: SPK-001 atau SPK-20260902-8K2"
                           class="w-full px-3.5 py-2 text-xs font-mono font-bold rounded-xl bg-white dark:bg-[#14161F] border border-[#E2E8F0] dark:border-[#252837] text-red-600 dark:text-red-400 uppercase tracking-wider focus:outline-none focus:ring-2 focus:ring-red-500/30">
                </div>

                <div class="grid grid-cols-1 sm:grid-cols-2 gap-3.5">
                    <!-- Armada Truk -->
                    <div class="sm:col-span-2">
                        <label class="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1">Pilih Armada Truk (Plat Nomor) <span class="text-rose-500">*</span></label>
                        <select name="kode_aset" x-model="formTambah.kode_aset" required
                                class="w-full px-3.5 py-2 text-xs rounded-xl bg-[#F8FAFC] dark:bg-[#1C1E2A] border border-[#E2E8F0] dark:border-[#252837] text-slate-800 dark:text-slate-200 focus:outline-none focus:ring-2 focus:ring-red-500/30">
                            <option value="">-- Pilih Truk Armada --</option>
                            @foreach($daftarKendaraan as $knd)
                                <option value="{{ $knd->kode_aset }}">
                                    {{ $knd->nama_aset }} ({{ $knd->merek_aset ?? 'Truk' }}) - {{ $knd->kode_aset }}
                                </option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <!-- Tanggal Masuk -->
                    <div>
                        <label class="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1">Tanggal Masuk Servis <span class="text-rose-500">*</span></label>
                        <input type="date" name="tanggal_masuk" x-model="formTambah.tanggal_masuk" required
                               class="w-full px-3.5 py-2 text-xs rounded-xl bg-[#F8FAFC] dark:bg-[#1C1E2A] border border-[#E2E8F0] dark:border-[#252837] text-slate-800 dark:text-slate-200 focus:outline-none focus:ring-2 focus:ring-red-500/30">
                    </div>

                    <!-- Tanggal Selesai -->
                    <div>
                        <label class="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1">Tanggal Selesai (Opsional)</label>
                        <input type="date" name="tanggal_selesai" x-model="formTambah.tanggal_selesai"
                               class="w-full px-3.5 py-2 text-xs rounded-xl bg-[#F8FAFC] dark:bg-[#1C1E2A] border border-[#E2E8F0] dark:border-[#252837] text-slate-800 dark:text-slate-200 focus:outline-none focus:ring-2 focus:ring-red-500/30">
                    </div>

                    <!-- Bengkel Pelaksana -->
                    <div>
                        <label class="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1">Bengkel Pelaksana <span class="text-rose-500">*</span></label>
                        <input type="text" name="bengkel_pelaksana" x-model="formTambah.bengkel_pelaksana" required placeholder="Contoh: Bengkel Internal PBJ / Bengkel Hino"
                               class="w-full px-3.5 py-2 text-xs rounded-xl bg-[#F8FAFC] dark:bg-[#1C1E2A] border border-[#E2E8F0] dark:border-[#252837] text-slate-800 dark:text-slate-200 focus:outline-none focus:ring-2 focus:ring-red-500/30">
                    </div>

                    <!-- Status Servis -->
                    <div>
                        <label class="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1">Status Pengerjaan <span class="text-rose-500">*</span></label>
                        <select name="status_perbaikan" x-model="formTambah.status_perbaikan" required
                                class="w-full px-3.5 py-2 text-xs rounded-xl bg-[#F8FAFC] dark:bg-[#1C1E2A] border border-[#E2E8F0] dark:border-[#252837] text-slate-800 dark:text-slate-200 focus:outline-none focus:ring-2 focus:ring-red-500/30">
                            <option value="Dalam Proses">Dalam Proses Pengerjaan</option>
                            <option value="Menunggu Sparepart">Menunggu Suku Cadang</option>
                            <option value="Selesai">Selesai Servis</option>
                ...
                <!-- Keluhan Kerusakan -->
                <div>
                    <label class="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1">Keluhan / Indikasi Kerusakan <span class="text-rose-500">*</span></label>
                    <textarea name="keluhan_kerusakan" x-model="formTambah.keluhan_kerusakan" rows="2" required placeholder="Deskripsikan keluhan pengemudi / kerusakan kendaraan..."
                              class="w-full px-3.5 py-2 text-xs rounded-xl bg-[#F8FAFC] dark:bg-[#1C1E2A] border border-[#E2E8F0] dark:border-[#252837] text-slate-800 dark:text-slate-200 focus:outline-none focus:ring-2 focus:ring-red-500/30"></textarea>
                </div>

                <!-- Tindakan Perbaikan -->
                <div>
                    <label class="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1">Tindakan Perbaikan / Suku Cadang yang Diganti</label>
                    <textarea name="tindakan_perbaikan" x-model="formTambah.tindakan_perbaikan" rows="2" placeholder="Rincian tindakan montir dan suku cadang yang diganti..."
                              class="w-full px-3.5 py-2 text-xs rounded-xl bg-[#F8FAFC] dark:bg-[#1C1E2A] border border-[#E2E8F0] dark:border-[#252837] text-slate-800 dark:text-slate-200 focus:outline-none focus:ring-2 focus:ring-red-500/30"></textarea>
                </div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-3.5">
                    <div>
                        <label class="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1">Biaya Ongkos Jasa (Rp)</label>
                        <input type="number" name="biaya_jasa" x-model.number="formTambah.biaya_jasa" step="1000" min="0" placeholder="0"
                               class="w-full px-3.5 py-2 text-xs font-mono font-bold rounded-xl bg-[#F8FAFC] dark:bg-[#1C1E2A] border border-[#E2E8F0] dark:border-[#252837] text-slate-800 dark:text-slate-200 focus:outline-none focus:ring-2 focus:ring-red-500/30">
                    </div>

                    <div>
                        <label class="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1">Biaya Sparepart / Suku Cadang (Rp)</label>
                        <input type="number" name="biaya_sparepart" x-model.number="formTambah.biaya_sparepart" step="1000" min="0" placeholder="0"
                               class="w-full px-3.5 py-2 text-xs font-mono font-bold rounded-xl bg-[#F8FAFC] dark:bg-[#1C1E2A] border border-[#E2E8F0] dark:border-[#252837] text-slate-800 dark:text-slate-200 focus:outline-none focus:ring-2 focus:ring-red-500/30">
                    </div>

                    <div class="sm:col-span-2">
                        <label class="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1">Pengawas Kendaraan Penanggung Jawab <span class="text-rose-500">*</span></label>
                        <input type="text" name="pengawas_kendaraan" x-model="formTambah.pengawas_kendaraan" required placeholder="Nama pengawas kendaraan"
                               class="w-full px-3.5 py-2 text-xs rounded-xl bg-[#F8FAFC] dark:bg-[#1C1E2A] border border-[#E2E8F0] dark:border-[#252837] text-slate-800 dark:text-slate-200 focus:outline-none focus:ring-2 focus:ring-red-500/30">
                    </div>
                </div>

                <!-- Live Total Biaya -->
                <div class="p-3.5 rounded-xl bg-amber-50 dark:bg-amber-500/10 border border-amber-200 dark:border-amber-500/20 flex items-center justify-between">
                    <div>
                        <span class="text-[11px] text-amber-800 dark:text-amber-300 font-semibold block">Total Estimasi Biaya Servis:</span>
                        <span class="text-[10px] text-amber-600 dark:text-amber-400 font-mono">Biaya Jasa + Biaya Sparepart</span>
                    </div>
                    <div class="text-right">
                        <strong class="font-mono font-bold text-base text-amber-700 dark:text-amber-300" x-text="'Rp ' + (totalBiayaTambah).toLocaleString('id-ID')"></strong>
                    </div>
                </div>
                ...
                <div class="flex items-center justify-end gap-2.5 pt-4 border-t border-[#E2E8F0] dark:border-[#252837]">
                    ...
                    <button type="submit"
                            class="px-5 py-2 text-xs font-semibold text-white bg-red-600 hover:bg-red-700 active:scale-95 rounded-xl transition-all shadow-md shadow-red-600/20">
                        Terbitkan SPK
                    </button>
                </div>
            </form>
    ...
    <!-- ========================================================================= -->
    <!-- 6. MODAL FORM: EDIT SPK PERBAIKAN -->
    <!-- ========================================================================= -->
        ...
        <div @click.away="modalEditTerbuka = false"
            ...
            <div class="flex items-center justify-between px-6 py-4 border-b border-[#E2E8F0] dark:border-[#252837]">
                <div class="flex items-center gap-2.5">
                    ...
                    <div>
                        <h2 class="text-base font-bold text-slate-900 dark:text-slate-100">Ubah Data SPK Perbaikan</h2>
                        <p class="text-[11px] text-slate-400">No. SPK: <span class="font-mono font-bold text-red-600" x-text="formEdit.nomor_spk_perbaikan"></span></p>
                    </div>
                </div>
            ...
            <form :action="'{{ url('operasional/bengkel/perbaikan') }}/' + formEdit.id_perbaikan" method="POST" class="p-6 space-y-4">
                @csrf
                @method('PUT')

                <div>
                    <label class="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1">Nomor SPK (Terkunci)</label>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-3.5">
                    <div class="sm:col-span-2">
                        <label class="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1">Armada Truk <span class="text-rose-500">*</span></label>
                    ...
                    <div>
                        <label class="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1">Tanggal Masuk Servis <span class="text-rose-500">*</span></label>
                        <input type="date" name="tanggal_masuk" x-model="formEdit.tanggal_masuk" required
                               class="w-full px-3.5 py-2 text-xs rounded-xl bg-[#F8FAFC] dark:bg-[#1C1E2A] border border-[#E2E8F0] dark:border-[#252837] text-slate-800 dark:text-slate-200 focus:outline-none focus:ring-2 focus:ring-amber-500/30">
                    </div>

                    <div>
                        <label class="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1">Tanggal Selesai</label>
                        <input type="date" name="tanggal_selesai" x-model="formEdit.tanggal_selesai"
                               class="w-full px-3.5 py-2 text-xs rounded-xl bg-[#F8FAFC] dark:bg-[#1C1E2A] border border-[#E2E8F0] dark:border-[#252837] text-slate-800 dark:text-slate-200 focus:outline-none focus:ring-2 focus:ring-amber-500/30">
                    </div>
                    ...
                    <div>
                        <label class="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1">Status Pengerjaan <span class="text-rose-500">*</span></label>
                        <select name="status_perbaikan" x-model="formEdit.status_perbaikan" required
                                class="w-full px-3.5 py-2 text-xs rounded-xl bg-[#F8FAFC] dark:bg-[#1C1E2A] border border-[#E2E8F0] dark:border-[#252837] text-slate-800 dark:text-slate-200 focus:outline-none focus:ring-2 focus:ring-amber-500/30">
                            <option value="Dalam Proses">Dalam Proses Pengerjaan</option>
                            <option value="Menunggu Sparepart">Menunggu Suku Cadang</option>
                            <option value="Selesai">Selesai Servis</option>
                ...
                <div>
                    <label class="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1">Keluhan / Kerusakan <span class="text-rose-500">*</span></label>
                ...
                <div>
                    <label class="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1">Tindakan Perbaikan</label>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-3.5">
                    <div>
                        <label class="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1">Biaya Jasa (Rp)</label>
                        <input type="number" name="biaya_jasa" x-model.number="formEdit.biaya_jasa" step="1000" min="0"
                    ...
                    <div>
                        <label class="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1">Biaya Sparepart (Rp)</label>
                        <input type="number" name="biaya_sparepart" x-model.number="formEdit.biaya_sparepart" step="1000" min="0"
                               class="w-full px-3.5 py-2 text-xs font-mono font-bold rounded-xl bg-[#F8FAFC] dark:bg-[#1C1E2A] border border-[#E2E8F0] dark:border-[#252837] text-slate-800 dark:text-slate-200 focus:outline-none focus:ring-2 focus:ring-amber-500/30">
                    </div>

                    <div class="sm:col-span-2">
                        <label class="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1">Pengawas Kendaraan <span class="text-rose-500">*</span></label>
    ...
    <!-- ========================================================================= -->
    <!-- 7. MODAL DETAIL & CETAK DOKUMEN SPK PERBAIKAN -->
    <!-- ========================================================================= -->
