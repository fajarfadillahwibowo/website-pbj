...
<div x-data="kelolaDriver()" x-init="initDriver()" class="space-y-6">
    ...
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
        ...
        <div class="bg-white dark:bg-[#14161F] p-4 rounded-2xl border border-[#E2E8F0] dark:border-[#252837] shadow-sm flex items-center gap-3.5">
            ...
            <div>
                <div class="text-[11px] font-medium text-slate-400 uppercase tracking-wider">Status Aktif</div>
        ...
        <div class="bg-white dark:bg-[#14161F] p-4 rounded-2xl border border-[#E2E8F0] dark:border-[#252837] shadow-sm flex items-center gap-3.5">
            ...
            <div>
                ...
                <div class="text-xl font-bold text-amber-600 dark:text-amber-400 font-mono mt-0.5">{{ $driverKontrak }} <span class="text-xs font-normal text-slate-400 font-sans">Orang</span></div>
            </div>
        </div>
        ...
        <div class="bg-white dark:bg-[#14161F] p-4 rounded-2xl border border-[#E2E8F0] dark:border-[#252837] shadow-sm flex items-center gap-3.5">
            ...
            <div>
                <div class="text-[11px] font-medium text-slate-400 uppercase tracking-wider">Non-Aktif / Cuti</div>
                <div class="text-xl font-bold text-slate-600 dark:text-slate-400 font-mono mt-0.5">{{ $driverNonaktif }} <span class="text-xs font-normal text-slate-400 font-sans">Orang</span></div>
            </div>
        </div>
    </div>

    <!-- 4. Tabel & Kontrol Pencarian / Filter -->
        ...
        <div class="p-4 sm:px-5 sm:py-4 border-b border-[#E2E8F0] dark:border-[#252837] flex flex-col md:flex-row md:items-center justify-between gap-3">
            <form method="GET" action="{{ route('operasional.armada.driver') }}" class="flex flex-col sm:flex-row items-stretch sm:items-center gap-2.5 flex-1 max-w-2xl">
                ...
                <select name="status" onchange="this.form.submit()"
                    ...
                    <option value="semua" {{ ($statusFilter ?? 'semua') === 'semua' ? 'selected' : '' }}>Semua Status Karyawan</option>
                    ...
                    <option value="berhenti" {{ ($statusFilter ?? '') === 'berhenti' ? 'selected' : '' }}>Berhenti</option>
                </select>
    ...
    <!-- ========================================================================= -->
    <!-- 5. MODAL FORM: TAMBAH DRIVER BARU -->
    <!-- ========================================================================= -->
        ...
        <div @click.away="modalTambahTerbuka = false"
            ...
            <div class="flex items-center justify-between px-6 py-4 border-b border-[#E2E8F0] dark:border-[#252837]">
                <div class="flex items-center gap-2.5">
                    ...
                    <div>
                        <h2 class="text-base font-bold text-slate-900 dark:text-slate-100">Tambah Data Driver Baru</h2>
                        <p class="text-[11px] text-slate-400">Lengkapi data profil, identitas KTP, dan berkas perjanjian kerja driver.</p>
                    </div>
                </div>
            ...
            <!-- Modal Form -->
            <form action="{{ route('operasional.armada.driver.simpan') }}" method="POST" enctype="multipart/form-data" class="p-6 space-y-4">
                @csrf
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        ...
                        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-2 mb-2">
                            ...
                            <div class="flex items-center gap-1.5 shrink-0">
                                <button type="button" @click="buatKodeOtomatis('gap')"
                                    ...
                                    <span>Isi Nomor Kosong (Daur Ulang)</span>
                                </button>
                                <button type="button" @click="buatKodeOtomatis('acak')"
                                    ...
                                    <span>Kode Acak Aman (Anti-Tebak)</span>
                                </button>
                            </div>
                        </div>
                    ...
                    <div>
                        ...
                        <select name="id_jabatan" x-model="formTambah.id_jabatan" required
                                class="w-full px-3.5 py-2 text-xs rounded-xl bg-[#F8FAFC] dark:bg-[#1C1E2A] border border-[#E2E8F0] dark:border-[#252837] text-slate-800 dark:text-slate-200 focus:outline-none focus:ring-2 focus:ring-blue-500/30">
                            <option value="">-- Pilih Jabatan --</option>
                            @foreach($daftarJabatan as $jab)
                                <option value="{{ $jab->id_jabatan }}">{{ $jab->nama_jabatan }} ({{ $jab->kode_jabatan }})</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div>
                        ...
                        <select name="status_karyawan" x-model="formTambah.status_karyawan" required
                                class="w-full px-3.5 py-2 text-xs rounded-xl bg-[#F8FAFC] dark:bg-[#1C1E2A] border border-[#E2E8F0] dark:border-[#252837] text-slate-800 dark:text-slate-200 focus:outline-none focus:ring-2 focus:ring-blue-500/30">
    ...
    <div x-show="modalEditTerbuka" x-cloak
        ...
        <div @click.away="modalEditTerbuka = false"
            ...
            <!-- Modal Form Edit -->
            <form :action="'{{ url('operasional/armada/driver') }}/' + formEdit.kode_karyawan" method="POST" enctype="multipart/form-data" class="p-6 space-y-4">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <div>
                        <label class="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1">Jabatan Karyawan <span class="text-rose-500">*</span></label>
                        <select name="id_jabatan" x-model="formEdit.id_jabatan" required
                                class="w-full px-3.5 py-2 text-xs rounded-xl bg-[#F8FAFC] dark:bg-[#1C1E2A] border border-[#E2E8F0] dark:border-[#252837] text-slate-800 dark:text-slate-200 focus:outline-none focus:ring-2 focus:ring-amber-500/30">
                            <option value="">-- Pilih Jabatan --</option>
                            @foreach($daftarJabatan as $jab)
                                <option value="{{ $jab->id_jabatan }}">{{ $jab->nama_jabatan }} ({{ $jab->kode_jabatan }})</option>
                    ...
                    <div>
                        <label class="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1">Status Karyawan <span class="text-rose-500">*</span></label>
                        <select name="status_karyawan" x-model="formEdit.status_karyawan" required
                                class="w-full px-3.5 py-2 text-xs rounded-xl bg-[#F8FAFC] dark:bg-[#1C1E2A] border border-[#E2E8F0] dark:border-[#252837] text-slate-800 dark:text-slate-200 focus:outline-none focus:ring-2 focus:ring-amber-500/30">
                            <option value="aktif">Aktif (Siap Operasi)</option>
                            <option value="kontrak">Kontrak</option>
                            <option value="tetap">Tetap</option>
                ...
                <!-- Upload Lampiran: Ganti Foto KTP & Ganti Dokumen Kontrak -->
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-2 border-t border-[#E2E8F0] dark:border-[#252837]">
                    ...
                    <div>
                        ...
                        <template x-if="formEdit.foto_ktp_url">
                            <div class="mt-2 flex items-center gap-2">
                                <span class="text-[10px] text-emerald-600 dark:text-emerald-400 font-medium">Foto saat ini terpasang:</span>
                                <a :href="formEdit.foto_ktp_url" target="_blank" class="text-[10px] text-blue-600 underline">Lihat Foto</a>
                            </div>
                        </template>
                    </div>
                    ...
                    <div>
                        ...
                        <template x-if="formEdit.file_kontrak_url">
                            <div class="mt-2 flex items-center gap-2">
                                <span class="text-[10px] text-emerald-600 dark:text-emerald-400 font-medium">Kontrak saat ini terpasang:</span>
                                <a :href="formEdit.file_kontrak_url" target="_blank" class="text-[10px] text-blue-600 underline">Buka Dokumen</a>
                            </div>
                        </template>
                    </div>
    ...
    <!-- ========================================================================= -->
    <!-- 7. MODAL DETAIL DRIVER (PROFIL LENGKAP) -->
    <!-- ========================================================================= -->
