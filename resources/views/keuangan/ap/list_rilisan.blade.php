@extends('layouts.app')

@section('judul', 'Daftar Rilisan Biaya Operasional (AP)')

@section('konten')
<div class="space-y-5">
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 bg-white dark:bg-[#14161F] p-4 sm:p-5 rounded-2xl border border-[#E2E8F0] dark:border-[#252837] shadow-sm">
        <div>
            <div class="text-xs text-rose-600 dark:text-rose-400 font-semibold font-mono uppercase tracking-wider mb-1">Account Payable · Dev 1</div>
            <h1 class="text-lg font-bold text-slate-900 dark:text-slate-100">Daftar Rilisan Pembayaran & Kas Bon</h1>
            <p class="text-xs text-slate-500 dark:text-slate-400 mt-0.5">Rilisan dana operasional sopir, kas bon darurat, dan klaim reimburse lapangan.</p>
        </div>
    </div>

    <div class="bg-white dark:bg-[#14161F] border border-[#E2E8F0] dark:border-[#252837] rounded-2xl overflow-hidden shadow-sm">
        <div class="overflow-x-auto">
            <table class="w-full text-xs">
                <thead class="bg-[#F8FAFC] dark:bg-[#1C1E2A] border-b border-[#E2E8F0] dark:border-[#252837] text-slate-500">
                    <tr>
                        <th class="px-4 py-2.5 text-left font-semibold uppercase tracking-wider">Tanggal Rilis</th>
                        <th class="px-4 py-2.5 text-left font-semibold uppercase tracking-wider">Penerima Dana</th>
                        <th class="px-4 py-2.5 text-right font-semibold uppercase tracking-wider">Nominal Rilis</th>
                        <th class="px-4 py-2.5 text-left font-semibold uppercase tracking-wider">Keterangan Keperluan</th>
                        <th class="px-4 py-2.5 text-center font-semibold uppercase tracking-wider">Status Validasi</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-[#EEF0F4] dark:divide-[#252837] text-slate-700 dark:text-slate-300">
                    <tr class="hover:bg-[#F8FAFC] dark:hover:bg-[#252837]/50 transition-colors">
                        <td class="px-4 py-3 font-mono">01/09/2026</td>
                        <td class="px-4 py-3 font-bold text-slate-900 dark:text-slate-100">Ahmad Supriyadi (Driver)</td>
                        <td class="px-4 py-3 text-right font-mono tabular-nums font-bold text-slate-900 dark:text-slate-100">Rp 1.500.000</td>
                        <td class="px-4 py-3 text-slate-500 dark:text-slate-400">Uang jalan kirim semen Gresik ke Karawang</td>
                        <td class="px-4 py-3 text-center"><span class="px-2 py-0.5 rounded text-[10px] font-semibold bg-emerald-50 dark:bg-emerald-500/10 text-emerald-700 dark:text-emerald-400">Disetujui SPV</span></td>
                    </tr>
                </tbody>
            </table>
        </div>
    </div>
</div>
@endsection
