...
<div class="space-y-5" x-data="{ 
    ...
    bukaModalEdit: false, 
    editData: {},
    kodeCustomerOtomatis: '{{ $kodeOtomatis }}',
    modeKode: 'gap',
    ...
    async buatKode(mode = 'gap') {
        ...
        try {
            const res = await fetch(`{{ route('master.customer.buat_kode') }}?mode=${mode}`);
            const data = await res.json();
            if (data.status === 'sukses') {
                this.kodeCustomerOtomatis = data.kode_otomatis;
                this.keteranganKode = data.keterangan;
            }
        } catch (e) {
            console.error('Gagal generate kode customer', e);
        } finally {
                    ...
                    @forelse($daftarCustomer ?? [] as $cust)
                        <tr class="hover:bg-[#F8FAFC] dark:hover:bg-[#252837]/50 transition-colors">
                            ...
                            <td class="px-4 py-3 text-center">
                                ...
                                <div class="mt-1 text-[10px] text-slate-400 dark:text-slate-500 font-mono" title="Terakhir diperbarui: {{ $cust->terakhir_diedit_waktu }}">
                                    🕒 {{ $cust->terakhir_diedit_relatif }}
                                </div>
                            </td>
                        </tr>
                    @empty
                ...
                <div class="grid grid-cols-2 gap-3">
                    <div>
                        <div class="flex items-center justify-between mb-1">
                            <label class="block font-semibold text-slate-700 dark:text-slate-300">Kode Customer</label>
                        ...
                        <input type="text" name="kode_customer" x-model="kodeCustomerOtomatis" required placeholder="CUST-001"
